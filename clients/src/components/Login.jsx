...
const Login = () => {
  ...
  return (
    <div className='fixed top-0 left-0 bottom-0 right-0 z-10 backdrop-blur-sm bg-black/30 flex 
    justify-center items-center'>
      ...
      <motion.form onSubmit={onSubmitHandler}
        ...
        viewport={{ onec: true }}
        className='relative bg-white/90 p-10 rounded-xl text-slate-500'>
        ...
        <button className='bg-blue-600 text-white w-full py-2 rounded-full cursor-pointer'>{state === 'Login' ?
          'Login' : 'create account'}</button>

        {state === 'Login' ? <p className='mt-5 text-center'>Dont have account?
          <span className='text-blue-600 cursor-pointer' onClick={() => (setState('Sign up'))}>Sign up</span></p>
          :
          <p className='mt-5 text-center'>Already have account?
