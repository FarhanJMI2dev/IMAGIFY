...
const BuyCredit = () => {
  ...
  const paymentRazorpay = async (planId) => {
    ...
    // console.log("Current User Status:", user, "Token:", token);
      ...
      // console.error("An error occurred in paymentRazorpay:", error);
      toast.error(error.message)
    }
  }
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        ...
        transition-all duration-500'>
            ...
            <button onClick={() => paymentRazorpay(item.id)}
              className='bg-gray-800 w-full text-white mt-8 text-sm rounded-md py-2.5 
            min-w-52 hover:cursor-pointer'>{user ? 'Purchase' : 'Get Started'}</button>
          </div>
        ))}
      </div>
